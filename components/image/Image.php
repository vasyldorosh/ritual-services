...
use Yii;
use app\components\image\drivers\ImageDriver;
...
class Image
{
    ...
    public function __construct($image, $config = NULL)
    {
        ...
        if ($check === FALSE)
            die('image getimagesize missing');
        ...
        if (!is_array($image_info) OR count($image_info) < 3)
            die($image . ': image file unreadable');
        ...
        $this->driver = new \app\components\image\drivers\GD($this->config['params']);

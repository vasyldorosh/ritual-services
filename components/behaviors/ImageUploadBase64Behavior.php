...
class ImageUploadBase64Behavior extends Behavior
{
    // Master Dimension
    const NONE = 1;
    const AUTO = 2;
    ...
    public function attach($owner)
    {
        if ($this->checkScenario())
        {
			...
			foreach($this->attributeName as $attrName){
                if (!empty($post[$class][$attrName]))
					$owner->validators[] = Validator::createValidator('safe', $owner, $attrName, []);
            }
        }		
    ...
    public function beforeValidate($event)
    {
	   foreach($this->attributeName as $attrName){
            ...
            if (!empty($this->owner->$attrName) && (empty($this->_oldImage[$attrName]) || ($this->owner->$attrName != $this->_oldImage[$attrName]))) {
				...
				if (!$base64 == base64_encode(base64_decode($base64))) {
					...
					if ($is_image===false) {
						$error = t('Файл не является изображением');
					}					
				}
    ...
    public function beforeSave($event)
    {
        foreach($this->attributeName as $attrName){
		    ...
		    if (!empty($this->owner->{$attrName}) && is_array($this->owner->$attrName))
            {
                $this->saveImage($attrName);
                $this->deleteImage($attrName);
            } else if (!empty($this->owner->{'is_delete_'. $attrName})) {
				$this->deleteImage($attrName);
			}
        }
    }
    ...
    public function saveImage($attrName)
    {
        ...
        file_put_contents($newFile, $data['content']);
     ...
     * @return string
     */
    public function getImageUrl($attr_name, $width = 0, $height = 0, $mode='resize')
    {
        return  (
        ($width > 0 || $height > 0) && (
        $thumbnail = $this->makeThumbnail($attr_name,$width, $height, $mode)
        ) !== null
            ? $thumbnail
